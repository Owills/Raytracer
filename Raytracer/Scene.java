...
public class Scene{
   ...
   public boolean isShadowed(Point p, Light li,double time){
       li.getRandomPoint();
       Ray r = new Ray(p,(li.computeLightDirection(p)),time);
   ...
   public Color computeVisibleColor(Ray r, int bouncesLeft){
       ...
       for(int j = 0; j< lights.size(); j++){
           if(!isShadowed(closest.getPosition(),lights.get(j),r.getTime())){
       ...
       Ray newR = new Ray(closest.getPosition(),M,r.getTime());
   ...
   public ColorImage render(int xRes, int yRes, int numSamples){
       //System.out.println("pixel: " +xRes +", " +yRes + ".");
